...
class CollectMineralShards(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if _MOVE_SCREEN in obs.observation["available_actions"]:
            ...
            if self.isSamePosition(oldPositions[self.selected], self.positions[self.selected]) or not self.targets[self.selected] or not self.isMineralShard(self.targets[self.selected]):
    ...
    def scoreTarget(self, target):
        ...
        if None not in self.targets:
            targetsDist = numpy.linalg.norm(numpy.array(self.targets[self.selected]) - numpy.array(self.targets[1-self.selected]))
