...
class CoordinatesHelper():
    ...
    def getCameraMove(self, location):
        x_cam = location.x + self._calibration.cameraObsOffset['x']
        y_cam = location.y + self._calibration.cameraObsOffset['y']
    ...
    def getRandomMinimapLocation(self):
        loc = Location()
        loc.x = np.random.randint(self.obsRange['x']['min'], self.obsRange['x']['max'])
        loc.y = np.random.randint(self.obsRange['y']['min'], self.obsRange['y']['max'])
        return loc
    ...
    def minimapSize(self):
        return self._calibration.minimapSize

    @property
    def fieldOfViewMap(self):
        return self._calibration.fieldOfViewMap

    @property
    def fieldOfViewMinimap(self):
        return self._calibration.fieldOfViewMinimap

    @property
    def obsRange(self):
        return self._calibration.obsRange
...
class Calibration():
    ...
    def step(self, obs):
        ...
        if not self._minimapSize['x'] or not self._minimapSize['y']:
            self._minimapSize['x'] = len(obs.observation["minimap"][MINI_VISIBILITY])
            self._minimapSize['y'] = len(obs.observation["minimap"][MINI_VISIBILITY][0])
        ...
        return actions.FunctionCall(MOVE_CAMERA, [[self._cameraPos['x'], self._cameraPos['y']]])