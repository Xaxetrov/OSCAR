    ...
    def step(self, obs, locked_choice=None):
        ...
        if self._state == ArmySupplier._BUILDING_BARRACKS:
            if not self._worker_selected:
                ...
                if building_center:
                    ...
                    self._shared['army'].add_barracks(obs, self._shared,
                                                      Location(
                                                          camera_loc=self._shared['camera'].location(obs, self._shared),
                                                          screen_loc=building_center))
                else:
            ...
            if not self._barracks_selected:
                on_screen_barracks = self._shared['screen'].scan_units(obs, self._shared, [TERRAN_BARRACKS_ID],
                                                                       PLAYER_SELF)
                ...
                if len(on_screen_barracks) == 0:
                    ...
                    play['actions'] = [actions.FunctionCall(SELECT_POINT,
                                                            [NEW_SELECTION,
                                                             selected_barracks.location.screen.get_flipped().to_array()])]
